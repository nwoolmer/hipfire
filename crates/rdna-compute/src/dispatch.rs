...
impl Gpu {
    ...
    /// HFQ1-G128 batched residual GEMM (WMMA i8 tiles). gfx1100+/gfx115x.
    /// Used for wo (attn output) and w_down (FFN down) projections in
    /// the prefill batched path.
    pub fn gemm_hfq1g128_residual_wmma(
        &mut self,
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "gemm_hfq1g128_residual_wmma",
            kernels::GEMM_HFQ1G128_RESIDUAL_WMMA_SRC,
            "gemm_hfq1g128_residual_wmma",
        )?;
        ...
        let func = &self.functions["gemm_hfq1g128_residual_wmma"];
        unsafe {
            self.hip.launch_kernel(
                func,
    ...
    /// HFQ1-G128 dp4a GEMV: matches PrismML's vec_dot_q1_0_q8_1 math.
    /// Pre-quantizes x → Q8_1 mmq blocks via the shared scratch path,
    /// then runs the dp4a-against-INT8 inner loop. Empirically tested vs
    /// the FP-direct multirow-quad — kept as a perf-comparison artifact.
    pub fn gemv_hfq1g128_dp4a(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        self.ensure_kernel(
            "gemv_hfq1g128_dp4a",
            kernels::GEMV_HFQ1G128_DP4A_SRC,
            "gemv_hfq1g128_dp4a",
        )?;
        let func = &self.functions["gemv_hfq1g128_dp4a"];
    ...
    /// HFQ1-G128 batched 3-way fused QKV GEMM (WMMA i8 tiles).
    /// gfx1100+ / gfx115x. X is converted to FP16 via the shared scratch.
    pub fn gemm_qkv_hfq1g128_wmma(
        &mut self,
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "gemm_qkv_hfq1g128_wmma",
            kernels::GEMM_QKV_HFQ1G128_WMMA_SRC,
            "gemm_qkv_hfq1g128_wmma",
        )?;
        let x_f16_ptr = self.ensure_fp16_x(x, batch_size * k)?;
        ...
        let func = &self.functions["gemm_qkv_hfq1g128_wmma"];
        unsafe {
            self.hip.launch_kernel(
                func,
                [row_tiles as u32, batch_tiles as u32, 1],
    ...
    /// HFQ1-G128 batched 2-way fused gate+up GEMM (WMMA).
    pub fn gemm_gate_up_hfq1g128_wmma(
        &mut self,
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "gemm_gate_up_hfq1g128_wmma",
            kernels::GEMM_GATE_UP_HFQ1G128_WMMA_SRC,
            "gemm_gate_up_hfq1g128_wmma",
        )?;
        let x_f16_ptr = self.ensure_fp16_x(x, batch_size * k)?;
        ...
        let func = &self.functions["gemm_gate_up_hfq1g128_wmma"];
        unsafe {
            self.hip.launch_kernel(
                func,
                [row_tiles as u32, batch_tiles as u32, 1],
    ...
    // ──────────────────────────────────────────────────────────────────────
    // MQ2-Lloyd MoE indexed family — 2-bit Lloyd-codebook siblings of the
    // HFQ4 indexed kernels above. Caller must pre-rotate X via FWHT before
    // these dispatches (MQ-family weights are FWHT-rotated at quant time).
    // ──────────────────────────────────────────────────────────────────────

    /// Index-aware MoE gate_up GEMV for MQ2-Lloyd. Sibling of
    /// `gemv_hfq4g256_moe_gate_up_k8_indexed`. `x` must be FWHT-rotated.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq2g256_lloyd_moe_gate_up_k8_indexed(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x_rot: &GpuTensor,
        ...
        // 72 B/group × M × (K/256) for weights + K*4 input + M*4 output × 8 experts.
        let weight_bytes = m * (k / 256) * 72;
        let bytes = 8 * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq2g256_lloyd_moe_gate_up_k8_indexed", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_gate_up_k8_indexed",
            [m as u32, 8, 1], [32, 1, 1], 0, &mut params,
    ...
    /// Index-aware MoE down GEMV with scaled residual for MQ2-Lloyd.
    /// Sibling of `gemv_hfq4g256_moe_down_residual_scaled_k8_indexed`.
    /// `rot_batch` must be FWHT-rotated per top-k slice.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        topk_weights: &GpuTensor,
        ...
        let bytes = 8 * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed",
            [m as u32, 8, 1], [32, 1, 1], 0, &mut params,
    ...
    /// N-batched MoE gate_up GEMV for MQ2-Lloyd. Sibling of
    /// `gemv_hfq4g256_moe_gate_up_k8_indexed_batched`.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq2g256_lloyd_moe_gate_up_k8_indexed_batched(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x_rot: &GpuTensor,
        ...
        let weight_bytes = m * (k / 256) * 72;
        let bytes = batch_size * k_top * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq2g256_lloyd_moe_gate_up_k8_indexed_batched", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_gate_up_k8_indexed_batched",
    ...
    /// N-batched MoE down GEMV + scaled residual for MQ2-Lloyd. Sibling of
    /// `gemv_hfq4g256_moe_down_residual_scaled_k8_indexed_batched`.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed_batched(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        topk_weights: &GpuTensor,
        ...
        let bytes = batch_size * k_top * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed_batched", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed_batched",
            [m as u32, k_top as u32, batch_size as u32], [32, 1, 1], 0, &mut params,
    ...
    // ──────────────────────────────────────────────────────────────────────
    // MQ3-Lloyd MoE indexed family — 3-bit Lloyd-codebook siblings of the
    // MQ2-Lloyd MoE wrappers above. 112 B/group (3.5 bpw) — 52 % larger
    // weights, materially better quality (validated MQ3-Lloyd-dense Qwen3.5).
    // Caller must pre-rotate X via FWHT, same contract as MQ2-Lloyd MoE.
    // ──────────────────────────────────────────────────────────────────────

    /// Index-aware MoE gate_up GEMV for MQ3-Lloyd.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq3g256_lloyd_moe_gate_up_k8_indexed(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x_rot: &GpuTensor,
        ...
        // 112 B/group × M × (K/256) for weights + K*4 input + M*4 output × 8 experts.
        let weight_bytes = m * (k / 256) * 112;
        let bytes = 8 * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq3g256_lloyd_moe_gate_up_k8_indexed", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq3g256_lloyd_moe_gate_up_k8_indexed",
            [m as u32, 8, 1], [32, 1, 1], 0, &mut params,
    ...
    /// Index-aware MoE down GEMV with scaled residual for MQ3-Lloyd.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        topk_weights: &GpuTensor,
        ...
        let bytes = 8 * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed",
            [m as u32, 8, 1], [32, 1, 1], 0, &mut params,
    ...
    /// N-batched MoE gate_up GEMV for MQ3-Lloyd.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq3g256_lloyd_moe_gate_up_k8_indexed_batched(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x_rot: &GpuTensor,
        ...
        self.ensure_kernel(
            ...
            kernels::GEMV_MQ3G256_LLOYD_MOE_GATE_UP_INDEXED_BATCHED_SRC,
        ...
        let bytes = batch_size * k_top * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq3g256_lloyd_moe_gate_up_k8_indexed_batched", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq3g256_lloyd_moe_gate_up_k8_indexed_batched",
            [m as u32, k_top as u32, batch_size as u32], [32, 1, 1], 0, &mut params,
    ...
    /// N-batched MoE down GEMV + scaled residual for MQ3-Lloyd.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed_batched(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        topk_weights: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "gemv_mq3g256_lloyd_moe_down_indexed_batched",
            kernels::GEMV_MQ3G256_LLOYD_MOE_DOWN_INDEXED_BATCHED_SRC,
            "gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed_batched",
        )?;
        let pp  = expert_ptrs.buf.as_ptr();
        let ip  = topk_indices.buf.as_ptr();
        let wp  = topk_weights.buf.as_ptr();
        ...
        let bytes = batch_size * k_top * (weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed_batched", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq3g256_lloyd_moe_down_residual_scaled_k8_indexed_batched",
            [m as u32, k_top as u32, batch_size as u32], [32, 1, 1], 0, &mut params,
