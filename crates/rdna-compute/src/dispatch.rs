...
impl Gpu {
    ...
    pub fn precompile_qwen35(&mut self, weight_quant: &str, kv_type: &str, head_dim: usize) -> HipResult<()> {
        ...
        // F16-weight × F32-input GEMV: used by V4F compressor/indexer F16
        // weights and any future full-precision MTP/dense paths.
        specs.push(("gemv_f16_xf32", kernels::GEMV_F16_XF32_SRC.to_string()));
    ...
    /// V4F MoE gate_up — POSITION-BATCHED MQ2-Lloyd K4-UNROLLED variant.
    /// Same shape contract and grid as
    /// `v4f_gemv_mq2g256_lloyd_moe_gate_up_indexed_batched` but the
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn v4f_gemv_mq2g256_lloyd_moe_gate_up_indexed_batched_k4(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x_rot: &GpuTensor,
        ...
        let bytes = batch_size * (k_top as usize) * (mq2_weight_bytes + k * 4 + m * 4);
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv", "v4f_gemv_mq2g256_lloyd_moe_gate_up_indexed_batched_k4", bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_gate_up_k8_indexed_batched_k4",
            [m as u32, k_top as u32, batch_size as u32], [32, 1, 1], 0, &mut params,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn v4f_gemv_mq2g256_lloyd_moe_down_residual_scaled_indexed_batched_k4(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        topk_weights: &GpuTensor,
        ...
        let timer = crate::profile::begin_timer(
            &self.hip, "gemv",
            "v4f_gemv_mq2g256_lloyd_moe_down_residual_scaled_indexed_batched_k4",
            bytes,
        );
        let result = self.launch_maybe_blob(
            "gemv_mq2g256_lloyd_moe_down_residual_scaled_k8_indexed_batched_k4",
